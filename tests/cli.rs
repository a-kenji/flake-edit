...
/// the active variant (flip-back), the `t` alias, and two-arg synthesis.
#[rstest]
...
/// Stage 2 of the matching ladder: a local checkout whose git remote
/// parses to the same repository as an input's variant resolves to that
/// input, even when the directory name matches nothing.
#[test]
...
/// Stage 3 of the matching ladder: a plain directory (not a git checkout)
/// resolves through its basename matching an input id.
#[test]
...
/// Stage 1 of the matching ladder beats the later stages: a path ref that
/// equals a stored variant (after canonicalization) flips that variant
