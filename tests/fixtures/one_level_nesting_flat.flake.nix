{
  ...
  #
  outputs =
    { self, nixpkgs, flake-utils, flake-utelinos, rust-overlay, crane, }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        rustFmtToolchainTOML =
          rustPkgs.rust-bin.fromRustupToolchainFile RUSTFMT_TOOLCHAIN;
        ...
        fmtInputs = [ pkgs.alejandra pkgs.treefmt pkgs.taplo pkgs.typos ];
        editorConfigInputs = [
          # pkgs.editorconfig-checker
        ];
        actionlintInputs = [ pkgs.actionlint pkgs.shellcheck ];
      ...
      in rec {
        devShells = {
          ...
          editorConfigShell =
            pkgs.mkShell { buildInputs = editorConfigInputs; };
        ...
        packages = {
          ...
          upstream =
            (pkgs.makeRustPlatform { inherit cargo rustc; }).buildRustPackage {
              ...
              inherit assetDir buildInputs cargoLock meta name postInstall src
                stdenv;
            };
          crane = craneLib.buildPackage (commonArgs // {
            ...
            inherit assetDir buildInputs cargoArtifacts meta
              # name
