...
pub(crate) fn color_diff_lines(diff: &str) -> Vec<Line<'_>> {
    diff.lines()
        .map(|line| {
...
pub(crate) fn list_height(item_count: usize, max_height: u16) -> u16 {
...
pub(crate) fn diff_height(line_count: usize) -> u16 {
...
pub(crate) mod layouts {
    ...
    pub(crate) fn content_with_footer(area: Rect) -> (Rect, Rect) {
        let chunks = Layout::vertical([Constraint::Min(3), Constraint::Length(1)]).split(area);
        (chunks[0], chunks[1])
    }

    /// Split area into fixed-height content area and footer bar
    pub(crate) fn fixed_content_with_footer(area: Rect, content_height: u16) -> (Rect, Rect) {
    ...
    pub(crate) fn footer_columns(area: Rect) -> Rc<[Rect]> {
    ...
    pub(crate) fn content_with_diff_preview(area: Rect, diff_lines: usize) -> (Rect, Rect) {
