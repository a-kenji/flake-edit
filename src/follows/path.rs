...
//!
//! `Segment` is a single attribute name. `AttrPath` is a non-empty sequence of
//! segments. Both store their values unquoted; the source-form `"..."` quotes
//! that Nix requires for names containing dots, leading digits, etc., live on
//! the rendering boundary (`Display` / `Segment::render`).
...
impl Segment {
    ...
    /// Everything else (including `.`, `+`, `/`, leading digits, hyphens,
    /// single quotes) is accepted; rendering decides whether to wrap in
    /// `"..."`.
    ...
    /// Parse source-form text. If wrapped in a single pair of `"..."`, strips
    /// the wrapping; otherwise behaves like [`Segment::from_unquoted`].
    ...
    /// (leading digit, embedded `.`, leading `-`, etc.) needs quoting.
...
impl AttrPath {
    ...
    /// All segments except the last, or `None` if the path has length 1.
    ...
    /// Returns true iff `self` is a structural prefix of `other` (a path is
    /// its own prefix).
    ...
    /// If `base` is a strict prefix of `self`, returns the suffix; otherwise
    /// `None` (also `None` when the paths are equal).
    ...
    /// `follows = [...]` arrays - `/` separators between segments with each
    /// segment unquoted.
...
mod tests {
    ...
    fn segment_from_syntax_via_rnix() {
        // Build a tiny CST containing an attrpath and pull a NODE_IDENT-like
        // child out. Using a directly constructed string node via parse.
        let src = r#"{ inputs."hls-1.10".url = "x"; }"#;
        let parsed = rnix::Root::parse(src);
        // Find NODE_STRING under the attrpath: the simplest verification is to
        // route a synthetic node through `from_syntax`.
        let syntax = parsed.syntax();
        // Locate the first NODE_STRING token-bearing node and feed it.
    ...
    fn segment_serde_roundtrip_dotted() {
        let s = Segment::from_unquoted("hls-1.10").unwrap();
        let j = serde_json::to_string(&s).unwrap();
        // Wire form is unquoted (no embedded backslash-quote).
    ...
    fn attr_path_serde_roundtrip() {
        ...
        // The wire form is the canonical Display output (quoted as needed).
