...
//!
//! The two share `FollowContext` plumbing in [`super::commands`].
...
pub fn add_follow(
    ...
    let change = if let (Some(input_val), Some(target_val)) = (input.clone(), target) {
        ...
        // Auto-deduplication (`auto::run`) bypasses this guard via direct
        // `apply_change` calls and is gated by `config.follow.max_depth`.
        let segments = input_id.path().len();
        if segments > 2 {
            return Err(CommandError::FlakeEdit(
                FlakeEditError::UnsupportedFollowsDepth {
