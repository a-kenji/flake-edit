...
//! - [`syntax`]: rnix parse errors and duplicate-attribute detection.
//! - [`follows`]: cycle / stale / target / contradiction / depth lints.
//!
//! [`validate`] runs the syntax-level lints; [`validate_full`] adds the
//! follows-graph lints that depend on a parsed [`InputMap`] and (optionally)
//! a [`FlakeLock`].
...
/// Bound on follows-graph traversal depth used by [`validate_full`]. Picked
/// large enough that real-world flakes never reach it, small enough to
/// short-circuit pathological graphs (e.g. lockfile cycles).
const GRAPH_MAX_DEPTH: usize = 64;

/// Run the syntax-level lints (parse errors, duplicate attributes, and the
/// always-on declared-cycle check) over `source`.
...
/// Full validation: syntax checks plus all follows-graph lints.
///
/// The follows-graph is built once from the parsed [`InputMap`] (and the
/// optional lockfile), then handed to each lint. One graph build per call
/// is cheap enough that callers do not need to share a pre-built graph.
pub fn validate_full(
    ...
    let graph = follows::build_graph(inputs, lock, GRAPH_MAX_DEPTH);
...
mod tests {
  ...
  inputs = { c.url = "c"; };
    ...
    fn validate_full_separates_warnings_from_errors() {
        // Stale follows is a warning; target-not-toplevel is an error. Both
